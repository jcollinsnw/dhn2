...
const pkToken = require("../src/token");
...
exports.got_launch = function(req, res) {
  token.getSharedSecret(consumer_secret => {
...
exports.caliper = function(req, res) {
  ...
  var parts = (caliper_profile_url_parts = url.parse(
    options.caliper_profile_url,
    true
  ));
  var caliper_profile_url_oauth =
...
exports.caliper_send = function(req, res) {
  finish(
    function(async) {
      ...
      var courseSectionId =
        "https://example.edu/politicalScience/2015/american-revolution-101/section/001";
      ...
      async("target", function(done) {
        ...
        var eventObj = new caliper.EPubVolume(
          "https://example.com/viewer/book/34843#epubcfi(/4/3)"
        );
        eventObj.setName(
        ...
        var target = new caliper.Frame(
          "https://example.com/viewer/book/34843#epubcfi(/4/3/1)"
        );
      ...
      async("navigatedFrom", function(done) {
        ...
        var navigatedFrom = new caliper.WebPage(
          "https://example.edu/politicalScience/2015/american-revolution-101/index.html"
        );
      ...
      async("edApp", function(done) {
        ...
        var edApp = new caliper.SoftwareApplication(
      ...
      async("group", function(done) {
        // LIS Course Offering
        var courseOffering = new caliper.CourseOffering(
          "https://example.edu/politicalScience/2015/american-revolution-101"
        );
        courseOffering.setName(
          "Political Science 101: The American Revolution"
        );
        courseOffering.setDescription(
          "Political Science 101: The American Revolution"
        );
        courseOffering.setCourseNumber("POL101");
        courseOffering.setAcademicSession("Fall-2015");
        courseOffering.setDateCreated(
          new Date("2015-08-01T06:00:00Z").toISOString()
        );
        courseOffering.setDateModified(
          new Date("2015-09-02T11:30:00Z").toISOString()
        );
        ...
        // LIS Group
        var group = new caliper.Group(
          "https://example.edu/politicalScience/2015/american-revolution-101/section/001/group/001"
        );
      ...
      async("membership", function(done) {
        ...
        var membership = new caliper.Membership(
          "https://example.edu/politicalScience/2015/american-revolution-101/roster/554433"
        );
    ...
    function(err, results) {
      ...
      console.log(
        "Sensor: %O Actor: %O Action: %O Object: %O Target: %O NavigatedFrom: %O EdApp: %O Group: %O Membership: %O",
        results["sensor"],
        results["actor"],
        results["action"],
        results["eventObj"],
        results["target"],
        results["navigatedFrom"],
        results["edApp"],
        results["group"],
        results["membership"]
      );
...
var send_outcomes = function(endpoint, sourced_id) {
  ...
  outcomes_service.send_replace_result(0.5, function(err, result) {
    console.log(result); //True or false
